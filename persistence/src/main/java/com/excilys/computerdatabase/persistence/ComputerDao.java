...
public class ComputerDao implements InterfaceDao<Computer> {
    ...
    public List<Computer> readSearch(String search, Map<String, String> orderMap) {
        JPQLQuery<Computer> query = queryFactory.get().selectFrom(computer).leftJoin(computer.company, company);
        if (search != null && !search.isEmpty()) {
            query = query.where(computer.name.contains(search).or(company.name.contains(search)));
        }
        query = orderBy(query, orderMap);
        return query.fetch();
    }
                ...
                if (entrySet.getKey() != null && !entrySet.getKey().isEmpty()) {
                    ...
                    case "name":
                        if (entrySet.getValue().equals("asc")) {
                            query = query.orderBy(computer.name.asc().nullsLast());
                        } else if (entrySet.getValue().equals("desc")) {
                            query = query.orderBy(computer.name.desc().nullsLast());
                        }
                        break;
                    case "introduced":
                        if (entrySet.getValue().equals("asc")) {
                            query = query.orderBy(computer.introducedDate.asc().nullsLast());
                        } else if (entrySet.getValue().equals("desc")) {
                            query = query.orderBy(computer.introducedDate.desc().nullsLast());
                        }
                        break;
                    case "discontinued":
                        if (entrySet.getValue().equals("asc")) {
                            query = query.orderBy(computer.discontinuedDate.asc().nullsLast());
                        } else if (entrySet.getValue().equals("desc")) {
                            query = query.orderBy(computer.discontinuedDate.desc().nullsLast());
                        }
                        break;
                    case "company":
                        if (entrySet.getValue().equals("asc")) {
                            query = query.orderBy(company.name.asc().nullsLast());
                        } else if (entrySet.getValue().equals("desc")) {
                            query = query.orderBy(company.name.desc().nullsLast());
                        }
                        break;
                    }
