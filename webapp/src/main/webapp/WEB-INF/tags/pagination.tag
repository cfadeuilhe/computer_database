...
<c:choose>
	<c:when test="${currentPage > 1}">
		<li><myTag:link cssClass="First" currentPage="1"
				...
				target="dashboard" text="&larr;" computerOrder="${computerOrder }"></myTag:link></li>
		<li><myTag:link cssClass="Previous"
				currentPage="${currentPage-1}" searchValue="${page.search }"
				pageSize="${pageSize}" target="dashboard" text="&laquo;" computerOrder="${computerOrder }"></myTag:link>
		</li>
	</c:when>
	<c:otherwise>
	...
	<c:if test="${item >= 1}">
		<c:if test="${item <= pageCount}">
			<li><myTag:link currentPage="${item}"
					searchValue="${page.search }" pageSize="${pageSize}"
					target="dashboard" text="${item}" computerOrder="${computerOrder }"></myTag:link></li>
		</c:if>
	</c:if>
</c:forEach>
...
<c:choose>
	<c:when test="${currentPage < pageCount}">
		<li><myTag:link cssClass="Next" currentPage="${currentPage+1}"
				searchValue="${page.search }" pageSize="${pageSize}"
				target="dashboard" text="&raquo;" computerOrder="${computerOrder }"></myTag:link></li>

		<li><myTag:link cssClass="Last" currentPage="${pageCount}"
				searchValue="${page.search }" pageSize="${pageSize}"
				target="dashboard" text="&rarr;" computerOrder="${computerOrder }"></myTag:link></li>
	</c:when>
	<c:otherwise>
		<li class="disabled"><myTag:link cssClass="Next"
				currentPage="${currentPage+1}" searchValue="${page.search }"
				pageSize="${pageSize}" target="dashboard" text="&raquo;" computerOrder="${computerOrder }"></myTag:link>
		</li>

		<li class="disabled"><myTag:link cssClass="Last"
				currentPage="${pageCount}" searchValue="${page.search }"
				pageSize="${pageSize}" target="dashboard" text="&rarr;" computerOrder="${computerOrder }"></myTag:link></li>
